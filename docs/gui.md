...
`open_browser(workspace)` creates one `BrowserState`, stores that workspace, and runs the render loop.
The first frame that can lock the workspace loads tags and `databrowser.toml` for its source. With
