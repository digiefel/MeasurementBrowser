...
function _collection_path_key(node::HierarchyNode)::String
    ...
    return collection_path_key(first(node.measurements).collection)
end
...
function _collection_location(node::HierarchyNode)::Vector{String}
    isempty(node.measurements) && error("Leaf node '$(node.name)' has no measurements")
    return copy(first(node.measurements).collection)
end
...
function _measurement_is_visible(
    ...
    _show_bad_effective(state) && return true
    tag_state = _tag_state_or_error(state)
    collection_key = collection_path_key(measurement.collection)
...
function _project_visible_selection(
    ...
    selected_measurements = ItemRecord[]
    for measurement_id in workspace.selection.measurement_ids
        ...
        collection_path_key(measurement.collection) in visible_collection_keys || continue
    ...
    return selected_collections, selected_measurements, selected_path
end

"""
Return every measurement belonging to the selected visible devices, ordered by time.
"""
function _measurements_of_selected_collections(
    state::BrowserState,
)::Vector{ItemRecord}
    selected_collections, _, _ = _project_visible_selection(state)
    all_measurements = ItemRecord[]
    sizehint!(all_measurements, sum(length(device.measurements) for device in selected_collections; init=0))
    for device in selected_collections
        for measurement in device.measurements
            push!(all_measurements, measurement)
        end
    end
    sort!(all_measurements, by=item_timestamp_key)
