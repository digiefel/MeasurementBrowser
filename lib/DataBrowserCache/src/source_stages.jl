...
function store_source_identity!(
    cache::MemoryCacheDB, item::AbstractDataSourceItem, ::AbstractString=label(item),
)::Nothing
    source_key = source_item_key!(cache, id(item); mint=true)
    lock(cache.lock) do
...
function store_source_result!(cache::MemoryCacheDB, stage::PipelineStage, source_key::Int64)::Nothing
    state = CachedKeyedResultState(Int8(stage), source_key, Int8(RESULT_READY), source_key, nothing)
    lock(cache.lock) do
        cache.keyed_result_states[(Int8(stage), source_key)] = state
    end
    _stage_ledger_result!(cache.stage_ledger, (Int8(stage), source_key), state)
    return nothing
end
...
"""
function store_source_read!(cache::AbstractCacheDB, source_key::Int64, value)::Nothing
...
function read_payload(cache::AbstractCacheDB, source_key::Integer; stage::PipelineStage=SOURCE_READ)
...
function has_payload(cache::AbstractCacheDB, source_key::Integer; stage::PipelineStage=SOURCE_READ)::Bool
...
function source_complete(cache::AbstractCacheDB, source_key::Integer)::Bool
...
function cache_knows_source(cache::AbstractCacheDB, source_key::Integer)::Bool
...
function delete_source_item!(cache::AbstractCacheDB, source_key::Integer,
...
function _delete_source_identity!(cache::MemoryCacheDB, source_key::Int64)::Nothing
    lock(cache.lock) do
