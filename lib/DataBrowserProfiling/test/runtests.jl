...
# raises its profiling level, so `@time_dbg` sections now record.
...
@testset "DataBrowserProfiling" begin
    ...
    @testset "nested sections nest, and each task merges once" begin
        reset_debug_timings!()
        op(n) = @time_dbg "outer" begin
            @time_dbg "inner" sum(1:n)
        end
    ...
    @testset "a snapshot is independent of later work" begin
        reset_debug_timings!()
        fetch(Threads.@spawn (@time_dbg "a" sum(1:10)))
    ...
    @testset "take! returns and resets the master" begin
        reset_debug_timings!()
        fetch(Threads.@spawn (@time_dbg "x" sum(1:10)))
    ...
    @testset "finish! stops recording" begin
        ...
        fetch(Threads.@spawn (@time_dbg "after" sum(1:10)))
