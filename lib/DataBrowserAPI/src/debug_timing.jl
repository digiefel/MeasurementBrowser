...
# `_time_dbg_begin`/`_time_dbg_end` stubs below are never called and add nothing.
#
...
#
# `@time_dbg` is INTERNAL: it is deliberately not exported. Consuming modules pull
# it in with `using DataBrowserAPI: @time_dbg`; it never reaches DataBrowser's
# public API.

"""
    profile_level() -> Int

Active instrumentation level. `0` (the default) disables every `@time_dbg`
section and lets the compiler remove it entirely. `DataBrowserProfiling` raises
this at runtime to switch instrumentation on. A `@time_dbg level=N` section
...
_time_dbg_begin(@nospecialize(label)) = nothing
_time_dbg_end(@nospecialize(token)) = nothing
...
# block are expected to pass an explicit label).
_time_dbg_label(sym::Symbol) = String(sym)
function _time_dbg_label(ex::Expr)
    if ex.head === :call
        return _time_dbg_callee(ex.args[1])
    elseif ex.head === :. && length(ex.args) == 2 && ex.args[2] isa QuoteNode
        return String(ex.args[2].value)
    end
    return string(ex)
end
_time_dbg_label(x) = string(x)

_time_dbg_callee(sym::Symbol) = String(sym)
function _time_dbg_callee(ex::Expr)
    if ex.head === :. && length(ex.args) == 2 && ex.args[2] isa QuoteNode
        return String(ex.args[2].value)
    end
    return string(ex)
end
_time_dbg_callee(x) = string(x)

"""
    @time_dbg [level=N] [label] expr
...
Forms:
    ...
    @time_dbg expr                  # level 1, label derived from the call
    @time_dbg "label" expr          # level 1, explicit label
    @time_dbg level=2 expr          # level 2, label derived from the call
    @time_dbg level=2 "label" expr  # level 2, explicit label
...
macro time_dbg(args...)
    ...
    length(rest) == 1 || error(
        "@time_dbg expects a single expression, optionally preceded by `level=N` and a string label",
    )
    expr = rest[1]
    lbl = label === nothing ? _time_dbg_label(expr) : label

    return quote
        if profile_level() >= $(esc(level))
            local _time_dbg_token = _time_dbg_begin($lbl)
            try
                $(esc(expr))
            finally
                _time_dbg_end(_time_dbg_token)
            end
        else
            $(esc(expr))
        end
